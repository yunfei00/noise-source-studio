...
- 文件任务和预测结果表格均使用 `QTableView`、`QAbstractTableModel` 与筛选代理；运行期间按
  item id 增量通知变化行，不重建整表或自适应列宽。
- 结果表格使用 `QTableView`、`QAbstractTableModel` 和筛选代理，支持万级结果虚拟化展示、
  关键词/状态/组合/噪声源/可信度筛选、低置信度筛选、排序和 2～5 项对比。
- item 状态在 150 ms 窗口内合并，任务摘要每 350 ms 刷新；统计图仅在统计页可见时按需刷新，
  运行期间最多每 2 秒一次。
