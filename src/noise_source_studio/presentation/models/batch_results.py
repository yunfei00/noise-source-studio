...
class BatchResultFilterProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(  # noqa: N802
        ...
        if item is None:
            return False
        payload = item.result or {}
        haystack = f"{item.file_name} {item.file_path}".casefold()
        if self.keyword and self.keyword not in haystack:
            return False
...
__all__ = ["BatchResultFilterProxyModel", "BatchResultTableModel"]
